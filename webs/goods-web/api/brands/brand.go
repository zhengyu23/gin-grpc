...
import (
	...
	"xrUncle/webs/goods-web/api"
	"xrUncle/webs/goods-web/forms"
...
func BrandList(ctx *gin.Context) {
	...
	//for _, value := range rsp.Data[pnInt:pnInt*pSizeInt+pSizeInt] { // 已经有分页策略，改用下面这种
...
// GetBrandListByCategoryId 通过分类Id获取品牌列表
func GetBrandListByCategoryId(ctx *gin.Context) {
	id := ctx.Param("id")
	i, err := strconv.ParseInt(id, 10, 32)
	if err != nil {
		ctx.Status(http.StatusNotFound)
...
// NewCategoryBrand 新建品牌_分类聚合 CategoryID BrandID
